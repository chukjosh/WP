...
function highlight(code: string) {
  ...
  // Changed ${k} to $1 to inject the captured function name group
...
export default function CodeEditor() {
  ...
  const [showSidebar, setShowSidebar] = useState(false); // Default to false on mobile screens
    ...
    <section className="mx-auto w-full max-w-6xl px-3 sm:px-6 py-8">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-800 bg-[#0B0F19] text-[#E2E8F0] shadow-2xl flex flex-col font-sans select-none backdrop-blur-xl">
        ...
        <div className="flex items-center justify-between bg-[#111827]/80 px-4 py-3 text-xs text-slate-400 border-b border-slate-800/60 backdrop-blur-md">
          ...
          <div className="truncate px-4 text-xs font-medium tracking-wide text-slate-300 max-w-[180px] sm:max-w-none bg-slate-800/40 py-1 px-3 rounded-md border border-slate-700/30">
            {active.filename}
          </div>
          <button 
        ...
        <div className="flex flex-col md:flex-row flex-1 min-h-[500px] md:min-h-[620px] overflow-hidden relative">
          ...
          <AnimatePresence initial={false}>
            {showSidebar && (
              <motion.div 
                ...
                className="bg-[#0F172A]/90 md:bg-[#0F172A] border-b md:border-b-0 md:border-r border-slate-800/60 flex flex-col text-xs shrink-0 overflow-hidden w-full md:w-auto"
              >
                ...
                <div className="mt-1 flex-1 overflow-y-auto px-2 pb-3 flex md:flex-col gap-1 overflow-x-auto md:overflow-x-hidden scrollbar-none">
                  {SAMPLES.map((sample) => (
                    <button
                      key={sample.id}
                      onClick={() => {
                      ...
                      className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-left transition-all shrink-0 md:shrink border ${
                        activeId === sample.id 
          ...
          <div className="flex-1 flex flex-col bg-[#0B0F19] overflow-hidden min-w-0">
            ...
            <div className="flex items-center justify-between bg-[#0F172A]/60 border-b border-slate-800/60 overflow-x-auto scrollbar-none">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="flex flex-1 overflow-hidden relative min-h-[220px]">
                {/* Line Numbers Gutter */}
                ...
                {/* Editor Content Overlay Wrapper Canvas Frame */}
                <div className="relative flex-1 h-full overflow-hidden">
                  <pre
                    ref={preRef}
                    aria-hidden="true"
                    className="pointer-events-none absolute inset-0 overflow-auto whitespace-pre p-4 font-mono text-xs leading-6 text-slate-300"
                  ...
                  <textarea
                    ...
                    className="absolute inset-0 resize-none overflow-auto bg-transparent p-4 font-mono text-xs leading-6 text-transparent caret-sky-400 outline-none selection:bg-sky-500/20 whitespace-pre [color:transparent] [-webkit-text-fill-color:transparent]"
                  />
                </div>
              </div>

              {/* Upgraded Terminal Console Component Wrapper */}
              {active.response && (
                <div className="border-t border-slate-800/80 bg-[#0A0E17] flex flex-col h-[200px] sm:h-[240px] shrink-0">
                  <div className="bg-[#0F172A]/90 px-4 py-2 text-[11px] font-semibold text-slate-400 border-b border-slate-800/40 flex items-center gap-4">
                    <span className="text-sky-400 border-b-2 border-sky-500 pb-2 pt-1 px-0.5 flex items-center gap-2 cursor-pointer">
                      <Terminal className="h-3.5 w-3.5 text-sky-400" /> TERMINAL OUTPUT
                    </span>
                    <span className="text-slate-500 hover:text-slate-300 pb-2 pt-1 px-0.5 flex items-center gap-2 cursor-pointer transition-colors">
                      <Braces className="h-3.5 w-3.5" /> RESPONSE JSON
                    </span>
                  </div>
                  <div className="flex-1 overflow-auto p-4 font-mono text-xs text-emerald-400/90 bg-[#060911] leading-6">
                    <pre className="whitespace-pre-wrap sm:whitespace-pre">{active.response}</pre>
                  </div>
                </div>
              )}
